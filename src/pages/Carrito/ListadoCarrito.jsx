...
function ListadoCarrito() {
  ...
  const { carrito } = useCarrito() // uso el hook para obtener solo las publicaciones(id) y cantidades del carrito

  const {
    aumentarCantidad, disminuirCantidad, eliminarDelCarrito, vaciarCarrito,
  } = useCarrito()
  ...
  const handleDisminuirCantidad = async (idPublicacion) => {
    ...
    if (publicacion) {
      ...
      if (publicacion.cantidad > 1) {
        setPublicacionesCarrito(publicacionesCarrito.map((p) => (p.idPublicacion === idPublicacion
